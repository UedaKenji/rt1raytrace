...
class Raytrace(frame.Frame):
    ...
    def set_lines(self,
        ...
        None
        """
        def to2dim(arg:np.ndarray):
            arg = np.array(arg)
            if len(arg.shape) == 1:
                return arg.reshape((1,arg.size))
            else:
                return arg 
        ...
        self.r0       = to2dim(r0)
        self.phi0     = to2dim(phi0)
        self.z0     = to2dim(z0)
        self.v0_theta = to2dim(v0_theta)
        self.h0_theta = to2dim(h0_theta)
        self.im_shape = self.r0.shape

        self.ray_1st = Ray( Ve_theta0 = self.v0_theta,
                            Ho_theta0 = self.h0_theta,
                            R0        = self.r0 ,
                            Phi0      = self.phi0,
                            Z0        = self.z0,)
    ...
    def main(self,
        ...
        if (not 'Ve_cam'  in dir(self)) and ((not 'r0'  in dir(self)) ) : 
...
class Raytrace_lines(frame.Frame):
    def load_model(
        path:str, 
        is_plot:bool=False
        ) :
    ...
    def __init__(self,
        ...
        super().__init__(dxf_file,show_print)
        print('you have to "set_lines()" ext.')
